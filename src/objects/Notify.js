...
export default function Notify(title, message, type, duration) {
	...
	let tile = document.createElement('div');
		tile.classList.add('notification', 'notification-item', 'is-' + type, 'fadeInDown', 'group');
		tile.addEventListener('click', function(e) {
			...
			tile.classList.add('closing', 'fadeOutRight');
			setTimeout(function() {
				tile.remove();
			}, 280);
		});
	...
	}, 500);  // animation runs for 800ms (due to .fast), but rendering might need a ms more to prevent "jumping" of element
